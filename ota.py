...
def _html(ip, mode, msg):
    ...
    for f in files:
        if f in PROTECTED:
            ...
            listing += '<div class="file item"><span class="name">'+f+'</span> ' \
                       + '<a class="danger" href="/del?f='+f+'" onclick="return confirm(\'Delete '+f+'?\')">delete</a></div>'
    run_info = "IDLE" if not _RUN_ACTIVE else "ACTIVE · " + (_RUN_NAME or "")
    if msg is None: msg = ""

    h  = '<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ESP 32 OTA / Dev Testing</title>'
    h += "<style>"
    ...
    h += "html,body{height:100%}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.5 ui-monospace,SFMono-Regular,Menlo,Consolas,Monaco,monospace}"
    h += ".wrap{max-width:1100px;margin:24px auto;padding:0 16px}.headline{font-size:20px;font-weight:700;margin:0 0 14px}.meta{color:var(--muted);margin:6px 0 18px}"
    ...
    h += "label, input, button{font:inherit}input[type=text]{width:260px;background:var(--bg);color:var(--text);border:1px solid var(--line);border-radius:8px;padding:.5rem .6rem;outline:none}"
    h += "input[type=text]:focus{border-color:var(--accent)}button{background:var(--btn);color:var(--text);border:1px solid var(--line);border-radius:10px;padding:.55rem .9rem;cursor:pointer}"
    h += "button:hover{border-color:var(--accent)}.accent{border-color:var(--accent);background:linear-gradient(180deg,#0f1a2a,#0b1322)}a{color:var(--accent)}a:hover{color:var(--accent2)}"
    h += ".row{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin:.4rem 0}textarea{width:100%;min-height:320px;resize:vertical;background:#0a1018;color:var(--text);border:1px solid var(--line);border-radius:10px;padding:12px 14px;outline:none;line-height:1.45}"
    h += "textarea:focus{border-color:var(--accent)}.status{position:sticky;top:0;z-index:5;margin:0 0 12px;padding:10px 12px;border-radius:10px;border:1px solid var(--line);background:#0a111a88;backdrop-filter:blur(6px)}"
    h += ".status.ok{border-color:var(--ok);color:var(--ok)}.status.err{border-color:var(--err);color:var(--err)}.files{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:8px}"
    h += ".file.item{padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:#0a1018}.file .name{color:#dbe7ff}.file .danger{color:var(--err);text-decoration:none;margin-left:8px}.file .danger:hover{text-decoration:underline}"
    h += ".bar{display:flex;justify-content:space-between;align-items:center;margin:6px 0}.hint{color:var(--muted);font-size:12px}.count{color:var(--muted);font-size:12px}"
    h += "</style>"
    ...
    status_class = "status"
    if msg.startswith("OK:"): status_class += " ok"
    elif msg.startswith("ERR:") or msg.startswith("Exception"): status_class += " err"
    h += '<div class="'+status_class+'">'+(msg.replace("<","&lt;") if msg else "Ready.")+'</div>'

    # Paste & run
    h += '<div class="card"><h3 style="margin:0 0 10px">Paste code, save & run</h3><form method="POST" action="/save" id="saveform">'
    h += '<div class="row"><label>Save as:</label><input name="name" id="name" type="text" value="app.py" required>'
    h += '<label style="display:flex;gap:8px;align-items:center"><input type="checkbox" name="run" value="1" checked> Run immediately (background)</label>'
    h += '<button class="accent" type="submit" id="saveBtn">Save & Run</button><button type="submit" id="saveOnly">Save only</button></div>'
    h += '<textarea name="code" id="code" placeholder="# paste your MicroPython here"></textarea>'
    h += '<div class="bar"><span class="hint">Tip: ⌘/Ctrl + S to Save & Run</span><span class="count" id="count">0 lines, 0 chars</span></div></form></div>'

    # Run existing
    h += '<div class="card"><h3 style="margin:0 0 10px">Run existing</h3><form method="GET" action="/run" class="row"><input name="f" value="app.py" placeholder="filename.py"><button type="submit">Run</button></form></div>'
    ...
    # Logs
    h += '<div class="card"><h3 style="margin:0 0 10px">Logs</h3><pre id="log" style="max-height:260px;overflow:auto;white-space:pre-wrap;background:#0a1018;border:1px solid var(--line);border-radius:10px;padding:12px;margin:0"></pre></div>'

    # Shell
    h += '<div class="card"><h3 style="margin:0 0 10px">Shell</h3>'
    h += '<div class="row"><span class="hint">Type Python here. Enter or ⌘/Ctrl+Enter to run. Use <code>:reset</code> to clear state.</span></div>'
    h += '<textarea id="repl_in" placeholder="print(\'hello\')\\n2+2"></textarea>'
    h += '<div class="bar"><div class="hint">&nbsp;</div><button id="repl_btn" type="button">Run</button></div>'
    h += '<pre id="repl_out" style="max-height:260px;overflow:auto;white-space:pre-wrap;background:#0a1018;border:1px solid var(--line);border-radius:10px;padding:12px;margin:0"></pre>'
    h += '</div>'

    # JS
    h += '<script>(function(){'
    # editor counters + save hotkey
